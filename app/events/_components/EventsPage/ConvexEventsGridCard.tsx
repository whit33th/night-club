...
export default function ConvexEventsGridCard({
  ...
  // Check if event is in the past based on Warsaw timezone
  ...
  const eventDateTime = new Date(`${event.date}T${event.startAt || "00:00"}`);
  const isEventPast = isPast || eventDateTime < currentWarsawDate;

  return (
    <Link href={linkHref}>
      <ViewTransition name={`event-card-${event._id}`} key={event._id}>
        <motion.div
          className={`group relative flex flex-col p-5 ${isEventPast ? "opacity-75" : ""}`}
        ...
        >
          ...
          <motion.div
            className={`absolute bottom-0 left-0 z-10 flex min-h-16 w-full items-center justify-between gap-3 rounded-xl px-4 backdrop-blur backdrop-hue-rotate-180 ${
              isEventPast ? "bg-neutral-900/40" : "bg-neutral-900/25"
            }`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{
          ...
          >
            <div className="flex w-full items-center justify-between gap-3">
              <div className="flex flex-col">
                <h1
                  className={`font-bold ${isEventPast ? "text-neutral-300" : ""}`}
                >
                  {(index + 1).toString().padStart(2, "0")}. {event.title}
                </h1>
                <p
                  className={`text-xs ${isEventPast ? "text-neutral-500" : "text-neutral-400"}`}
                >
                  {artistsDisplay}
                </p>
              </div>
